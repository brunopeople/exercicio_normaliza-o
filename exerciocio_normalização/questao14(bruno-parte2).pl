...
% Use as linhas abaixo na janela do SWI-prolog para obter a resposta
% schema(R), fds(F), decomp(D), ljd(R,F,D).
% schema(R), fds(F), decomp(D), fpd(R,F,D).

% OU descomente a linha abaixo para usar answer1 e answer2 para ver a resposta
%answer1 :- schema(R), fds(F), decomp(D), ljd(R,F,D).
%answer2 :- schema(R), fds(F), decomp(D), fpd(R,F,D).
